...
{
    Cart, RelativeBase, RelativeTool
}

namespace Delta_robotin_ohjaus
{
    //***********************************************
    ...
    public class HardwareProtocolClient
    {
        ...
        public bool WaitForMove(int timeoutMs = 30000)
        {
            int elapsed = 0;
            while (CurrentState == RobotState.Moving && elapsed < timeoutMs)
            {
                Thread.Sleep(50);
                elapsed += 50;
            }
            ...
            {
                log.Error("WaitForMove timed out");
                CurrentState = RobotState.Error;
                return false;
            }

            return CurrentState != RobotState.Error;
        }

        public bool WaitForConnection(int timeoutMs = 10000)
        {
            int elapsed = 0;
            while (CurrentConnectionState == ConnectionState.Connecting && elapsed < timeoutMs)
            {
                Thread.Sleep(50);
                elapsed += 50;
            }
                        ...
                        try
                        {
                            ...
                            if (gotStart)
                            {
                                ...
                                if (gotEnd)
                                {
                                    ...
                                    if (!gotMessage) CurrentConnectionState = ConnectionState.Connected;
        ...
        public void MoveToCoord(motionType type, double x, double y, double z, double velmms)
        {
            string msg = "CMD Move " + 
                (type == motionType.Cart ? "Cart " : type == motionType.RelativeBase ? "RelativeBase " : "RelativeTool ") + 
                x.ToString("0.0") + " " +
                y.ToString("0.0") + " " +
                z.ToString("0.0") + " " +
        ...
        */
        ...
        private void ParseString(string msg)
        {
            try
            {
                ...
                else if (msgType == "EXECERROR")
                {
                    log.ErrorFormat("Move error: {0}", msg);
                    Console.WriteLine("EXECERROR: " + msg);
                    CurrentState = RobotState.Error;
                }
                ...
                else if (msgType == "EXECEND")
                {
                    bool success = msg.Contains("PLAN");
                    log.InfoFormat("Move complete, success: {0}, msg: {1}", success, msg);
                    Console.WriteLine("EXECEND: " + msg);
                    CurrentState = success ? RobotState.Idle : RobotState.Error;
                }
                ...
                else if (msgType == "CMDACK")
                {
                    log.InfoFormat("Command acknowledged: {0}", msg);
                }
                else if (msgType == "CMDERROR")
                {
                    log.ErrorFormat("Command error: {0}", msg);
                    Console.WriteLine("CMDERROR: " + msg);
                }
                ...
                else if (msgType == "CMD")
                {
                    ...
                    {
                        bool isActive = parts.Length > 4 && parts[4] == "true";
                        log.InfoFormat("Connection active state: {0}", isActive);
                        Console.WriteLine("Active state: " + isActive);

                        if (!isActive)
                        {
                            log.Info("Connection is passive, requesting active state...");
                            SendCommand("CMD SetActive true");
                        }
                    }
                }
                ...
                {
                    log.InfoFormat("Robot message: {0}", msg);
                    Console.WriteLine("ROBOT MSG: " + msg);
                }
